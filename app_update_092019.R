...
All_Studies_18409 <- read.delim("20190917101929AllDataAnnotated.txt", na = "empty")
...
#glimpse(All_Studies_18409)

## Data preparation 
modelAllStudies <- All_Studies_18409[, 1:56] 
#glimpse(modelAllStudies)
removeModel_rows <- modelAllStudies[apply(modelAllStudies[c(19:56)], 1, function(z) any (z!=0)),]
...
drugAllStudies <- All_Studies_18409[, c(1:18, 58:172)]
removeDrug_rows <- drugAllStudies[apply(drugAllStudies[c(19:133)], 1, function(z) any (z!=0)),]
...
## change column types
#removeModel$PdfPathStatus.x <- as.character(removeModel$PdfPathStatus.x)
#removeDrug$PdfPathStatus.x <- as.character(removeDrug$PdfPathStatus.x)
...
colnames(removeModel)[c(19:53)] <- sub("Regex.*", "", colnames(removeModel)[c(19:53)])
colnames(removeDrug)[c(19:108)] <- sub("Regex.*", "", colnames(removeDrug)[c(19:108)])
...
ui <- fluidPage(
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 pickerInput(inputId = "modelOptions", label= "Select models of depression to display",
                             choices = colnames(removeModel)[c(19:53)],
                             selected = colnames(removeModel)[c(19:53)],
           ...
           sidebarLayout(
             sidebarPanel(
               ...
               pickerInput(inputId = "drugOptions",
                           ...
                           choices = colnames(removeDrug)[c(19:108)],
                           selected = colnames(removeDrug)[c(19:108)],
                           options = list(`actions-box` = TRUE),
                           multiple = TRUE
               )
             ),
  ...
  helpText(h5(
    "The studies in this app were identified using a custom machine learning algorithm to identify animal models of depression. The approach is described", a(href="https://doi.org/10.1186/s13643-019-0942-7", "here.", target = "_blank"), "This search was last updated on July 2019 using the living systematic search on", a(href="https://app.syrf.org.uk", "SyRF.", target = "_blank")
  )),  
  br(), 
  ...
  helpText(h5(
    ...
    "from December, 2016. These dictionaries were built as part of a fellowship with",   a(href = "http://contentmine.org/", "ContentMine.", target = "_blank")
  )), 
  br(), 
  br() , 
  helpText(h3(
...
server <- function(input, output) {
  ...
  modelValsGraph <- reactive({
    a <- removeModel[, c('Keywords', input$modelOptions)]
  #  a <- removeModel[, c('depID', 'DEX', 'gluKO')]
  ...
  # qwang
  modelValsTable <- reactive({
    mname = input$modelOptions
    b = cbind(removeModel[,1:18],removeModel[,colnames(removeModel) %in% mname])
    c = b[rowSums(removeModel[,colnames(removeModel) %in% mname])!=0,]
    c <- droplevels(c)
    return(c)
  })
  ...
  drugValsGraph  <- reactive({
    b <- removeDrug[ , c('Keywords', input$drugOptions)]
    b <- droplevels(b)
    return(b)
  })
  ...
  drugValsTable <- reactive({
    ...
    dname = input$drugOptions
    b = cbind(removeDrug[,1:18],removeDrug[,colnames(removeDrug) %in% dname])
    c = b[rowSums(removeDrug[,colnames(removeDrug) %in% dname])!=0,]
    c <- droplevels(c)
    return(c)
  })
  ...
  output$modelPlot <- renderPlot({
    ...
    model_gath <- gather(modelValsGraph(), model, freq, -Keywords)
    ...
    mDep_group <- model_gath %>% group_by(model)%>% summarize(num_docs=n_distinct(Keywords), avg_freq=mean(freq))
    ...
    ####testing
#    model_gath <- gather(a, model, freq, -depID)
  ...
  output$drugPlot <- renderPlot({
    ...
    # wrangling the data
    drug_gath <- gather(drugValsGraph(), drug, freq, -Keywords)
    ...
    dDep_group <- drug_gath %>% group_by(drug) %>% summarize(num_docs=n_distinct(Keywords), avg_freq=mean(freq))
  ...
  output$xModel <- DT::renderDataTable({
    ...
    datatable(
      modelTabData[,c(1,3,12)], extensions = 'FixedHeader', 
  ...
  output$xAllData <- DT::renderDataTable({
    #table displays what user has selected in sidebar
    datatable(
      All_Studies_18409[,c(1,3,12)], extensions = 'FixedHeader', 
      options = list(pageLength = 10, fixedHeader = TRUE, orderClasses = TRUE,  
                     dom='Brtip'),  
      rownames = FALSE
    )
