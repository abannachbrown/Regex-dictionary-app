...
All_Studies_18409 <- All_Studies_18409 <- read_csv("all_all.csv", 
                                                   +     na = "empty")
...
## Data preparation 
modelAllStudies <- All_Studies_18409[, 1:59] 
glimpse(modelAllStudies)
removeModel_rows <- modelAllStudies[apply(modelAllStudies[c(22:59)], 1, function(z) any (z!=0)),]
...
drugAllStudies <- All_Studies_18409[, c(1:21, 61:176)]
removeDrug_rows <- drugAllStudies[apply(drugAllStudies[c(22:136)], 1, function(z) any (z!=0)),]
...
colnames(removeModel)[c(22:58)] <- sub("Regex.*", "", colnames(removeModel)[c(22:58)])
colnames(removeDrug)[c(22:126)] <- sub("Regex.*", "", colnames(removeDrug)[c(22:126)])
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 pickerInput(inputId = "modelOptions", label= "Select models of depression to display",
                             choices = colnames(removeModel)[c(22:58)],
                             selected = colnames(removeModel)[c(22:58)],
           ...
           sidebarLayout(
             sidebarPanel(
               ...
               pickerInput(inputId = "drugOptions",
                           ...
                           choices = colnames(removeDrug)[c(22:126)],
                           selected = colnames(removeDrug)[c(22:126)],
                           options = list(`actions-box` = TRUE),
                           multiple = TRUE
               )
             ),
...
server <- function(input, output) {
  ...
  modelValsTable <- reactive({
    ...
    b = cbind(removeModel[,1:21],removeModel[,colnames(removeModel) %in% mname])
  ...
  drugValsTable <- reactive({
    ...
    b = cbind(removeDrug[,1:21],removeDrug[,colnames(removeDrug) %in% dname])
